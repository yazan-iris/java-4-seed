...
public class DataHeader extends ControlHeader {
	...
	public DataHeader(Type type) {
		this(0, type);
	}

	public DataHeader(int sequence, Type type) {
		super(sequence, type, false);
	}

	public DataHeader(int sequence, Type type, boolean continuation) {
		super(sequence, type, continuation);
	}
	...
	public static class Builder {
		...
		public DataHeader build(byte[] bytes) throws SeedException {
			if (bytes == null) {
			...
			DataHeader header = new DataHeader(Integer.parseInt(new String(bytes, 0, 6)), Type.from((char) bytes[7]),
					((char) bytes[6] == '*'));
		...
		public DataHeader build(int sequence, Type type, boolean continuation) {
			return new DataHeader(sequence, type, continuation);
		}
