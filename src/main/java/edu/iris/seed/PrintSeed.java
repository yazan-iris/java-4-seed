...
public class PrintSeed {
	...
	public static void main(String[] args) throws Exception {
		...
		List<BlocketteDefinition> theList = new ArrayList<>();
		for (int i : controlMap.keySet()) {
			String s = describe(i);
			String[] lines = s.split(System.getProperty("line.separator"));
			String[] def = lines[0].split("\\t+");
			BlocketteDefinition d = new BlocketteDefinition();
			theList.add(d);
			d.setNumber(Integer.parseInt(def[0].trim()));
			d.setDescription(def[1].trim());
			d.setType(def[2].trim());
			d.setSince(def[3].trim());

			List<SeedField> list = new ArrayList<>();
			d.setFields(list);
			for (int l = 1; l < lines.length; l++) {
				String[] array = lines[l].split("\\t+");

				SeedField f = new SeedField();
				list.add(f);
				f.setDescription(array[1].trim());
				String[] lengthArray = array[3].trim().split("-");
				f.setMinumumLength(Integer.parseInt(lengthArray[0].trim()));
				if (lengthArray.length > 1) {
					f.setMaximumLength(Integer.parseInt(lengthArray[1].trim()));
				}

				f.setMask(array[4].trim());
				f.setNumber(Integer.parseInt(array[0].trim()));
				f.setType(array[2].trim());

			}
		}

		for (BlocketteDefinition d : theList) {
			// System.out.println(d);
		}
		ObjectMapper objectMapper = new ObjectMapper();
		System.out.println(objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(theList));
	}
